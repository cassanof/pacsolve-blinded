    ...
    def run(self):
        ...
        for package_name in pkgs:
            ...
            if not os.path.isdir(package_path):
                eprint(
                    f'Skipping {package_name} ({package_path} is not a directory)')
                continue
            parallel_experiment_path = os.path.join(
                package_path, 'experiment.json')
            if not os.path.isfile(parallel_experiment_path):
                eprint(
                    f'Skipping {package_name} ({parallel_experiment_path} is not a file)')
                continue
