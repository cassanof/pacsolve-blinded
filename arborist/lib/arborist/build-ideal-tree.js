...
const { ROSETTE_FUNCTIONS, CONSTRAINT_PRIMITIVES, sumCosts, linearCombination, CONSISTENCY_RELATIONS, min_duplicates } = require('./rosette-functions.js')
const { rosetteConstraintFromRange, parseVersionString, buildPrereleaseOrder, literalSemver, orderJsonVersion } = require('./rosette-translation-helpers.js')
...
const _reachableNodesQueue = Symbol('reachableNodesQueue')
const _buildReachableRegistry = Symbol('buildReachableRegistry')
const _setConsistencyOnQuery = Symbol('setConsistencyOnQuery')
const _reachableRegistry = Symbol('reachableRegistry')
const _seenNodes = Symbol('seenNodes')
const _foundContextNode = Symbol('foundContextNode')
...
const _useRosetteSolver = Symbol('useRosetteSolver')
const _rosetteSolverPath = Symbol('rosetteSolverPath')
const _rosetteConsistencyArg = Symbol('rosetteConsistencyArg')
const _rosetteDisallowCyclesArg = Symbol('rosetteDisallowCyclesArg')
const _rosetteMinimizeArg = Symbol('rosetteMinimizeArg')
const _rosetteOnlyExplore = Symbol('rosetteOnlyExplore')
...
const _solveTreeWithRosette = Symbol('solveTreeWithRosette')
const _buildRosetteQuery = Symbol('buildRosetteQuery')
const _reachableNodesStep = Symbol('reachableNodesStep')
const _markNodeSeen = Symbol('markNodeSeen')
const _performRosetteQuery = Symbol('performRosetteQuery')
const _decodeRosetteAnswer = Symbol('decodeRosetteAnswer')
const _nodeFromEdge = Symbol('nodeFromEdge')
const _nodeFromSpec = Symbol('nodeFromSpec')
const _nodeFromPkg = Symbol('nodeFromPkg')
const _fetchManifest = Symbol('fetchManifest')
const _fetchPackuments = Symbol('fetchPackuments')
const _installDevDependencies = Symbol('installDevDependencies')
...
module.exports = cls => class IdealTreeBuilder extends cls {
  constructor (options) {
    ...
    this[_useRosetteSolver] = false
    this[_rosetteSolverPath] = null
    this[_rosetteConsistencyArg] = null
    this[_rosetteDisallowCyclesArg] = false
    this[_rosetteMinimizeArg] = null
    this[_rosetteOnlyExplore] = false
  ...
  async buildIdealTree (options = {}) {
    ...
    try {
      ...
      if(this[_useRosetteSolver]) {
        await this[_solveTreeWithRosette]()
      } else {
  ...
  async [_solveTreeWithRosette] () {
    const { query, prereleaseOrders } = await this[_buildRosetteQuery]()

    if(this[_rosetteConsistencyArg] == 'pip-else-npm') {
      this[_setConsistencyOnQuery](query, 'pip')
      const rosetteAnswerPip = await this[_performRosetteQuery](query)

      if(rosetteAnswerPip.success) {
        this[_decodeRosetteAnswer](rosetteAnswerPip, prereleaseOrders)
      } else {
        console.log('solve with pip not ok, falling back to npm');
        this[_setConsistencyOnQuery](query, 'npm')
        const rosetteAnswerNpm = await this[_performRosetteQuery](query)
        this[_decodeRosetteAnswer](rosetteAnswerNpm, prereleaseOrders)
      }
    } else {
      this[_setConsistencyOnQuery](query, this[_rosetteConsistencyArg])
      const rosetteAnswer = await this[_performRosetteQuery](query)
      this[_decodeRosetteAnswer](rosetteAnswer, prereleaseOrders)
    }

    // await this[_installDevDependencies]()
  }

  async [_installDevDependencies] () {
    await this[_buildDeps]()
  }

  async [_buildRosetteQuery] () {

    const has_min_cve = this[_rosetteMinimizeArg].includes('min_cve');

    const rootNode = this.idealTree.target

    // Places the results in this[_reachableRegistry] and this[_foundContextNode]
    await this[_buildReachableRegistry](rootNode, has_min_cve)
    
    const reachableRegistry = this[_reachableRegistry]
    const contextNode = this[_foundContextNode]

    let allPrereleases = {}
    contextNode.forEach(dep => {
      let depPrereleases = []
      if(dep.semverRange) {
        depPrereleases = dep.semverRange.set.flatMap(conj => {
          return conj.flatMap(comp => {
            return comp.semver.prerelease ? [comp.semver.prerelease] : []
          })
        })
      }
      allPrereleases[dep.packageToDependOn] = allPrereleases[dep.packageToDependOn] || []
      allPrereleases[dep.packageToDependOn] = allPrereleases[dep.packageToDependOn].concat(depPrereleases)
    })

    Object.entries(reachableRegistry).forEach(([name, versions]) => {
      Object.entries(versions).forEach(([version, data]) => {
        if(data.semver) {
          allPrereleases[name] = allPrereleases[name] || []
          allPrereleases[name].push(data.semver.prerelease)
        }
    ...
    // Map PackageName PrereleaseOrderObj
    const allPrereleaseOrders = Object.fromEntries(Object.entries(allPrereleases).map(([name, prereleases]) => {
      return [name, buildPrereleaseOrder(prereleases)]
    }))

    const rosetteContextNode = contextNode.map(d => {
      return {'packageToDependOn': d.packageToDependOn, 'constraint': rosetteConstraintFromRange(d.semverRange, allPrereleaseOrders[d.packageToDependOn])}
    })
    

    let names = Object.keys(reachableRegistry)

    const rosetteRegistry = names.map(name => {
      const versions_obj = reachableRegistry[name]
      const prereleaseOrder = allPrereleaseOrders[name]

      let versions_data = Object.entries(versions_obj).flatMap(([versionString, data]) => {
        const jsonRosetteVersion = parseVersionString(versionString, prereleaseOrder) 
        const deps = data.dependencies
        
        if(!jsonRosetteVersion) {
          console.error(`Failed to parse version: ${versionString}`)
          return []
        }

        const rosetteDeps = deps.map(d => {
          const constraint = rosetteConstraintFromRange(d.semverRange, allPrereleaseOrders[d.packageToDependOn])
          return {'packageToDependOn': d.packageToDependOn, 'constraint': constraint}
        })
        return [{'version': jsonRosetteVersion, 'dependencies': rosetteDeps, 'cost_values': data.cost_values}]
      })

      // versions_data.sort((obj1, obj2) => orderJsonVersion(obj1.version, obj2.version))
      // versions_data.reverse()

      return {'package': name, 'versions': versions_data, 'cost_values': {'_unused': 0}}
    })



    const base_optimization_functions = {
      min_duplicates: min_duplicates,
      min_num_deps: sumCosts((pg) => dsl.helpers.c(0), (vn) => dsl.helpers.c(1)),
      min_oldness: sumCosts(
        (pg) => dsl.helpers.c(0), 
        (vn) => dsl.helpers.op('get-cost-val/version-node', vn, dsl.helpers.c('version_oldness'))),
      min_cve: sumCosts(
        (pg) => dsl.helpers.c(0), 
        (vn) => dsl.helpers.op('get-cost-val/version-node', vn, dsl.helpers.c('version_cve_badness')))
    }

    const disallow_cycles = this[_rosetteDisallowCyclesArg]
    const requested_mins_str = this[_rosetteMinimizeArg] //'min_num_deps , 0.5 * min_oldness'

    const requested_optimization_function_descriptions = requested_mins_str.split(',').map(s => s.trim()).map(s => {
      const terms = s.split('+').map(s => s.trim()).map(t_str => {
        if(t_str.includes('*')) {
          const [coeff, base] = t_str.split('*').map(s => s.trim())
          return {base, coeff: parseFloat(coeff)}
        } else {
          return {base: t_str.trim(), coeff: 1}
        }
      })

      if(terms.length === 1) {
        return {type: 'single', base: terms[0].base}
      } else if(terms.length === 0) {
        throw new Error(`Invalid optimization function description: ${t_str}`)
      } else {
        return {type: 'linear_combination', terms}
      }
    })

    const requested_optimization_function_names = requested_optimization_function_descriptions.map((desc) => {
      if(desc.type == 'single') {
        if(desc.base in base_optimization_functions) {
          return desc.base
        } else {
          throw new Error(`Unknown optimization function: ${desc.base}`)
        }
      } else {
        if(desc.terms.length == 0) {
          throw new Error('Linear combination optimization function must have at least one term')
        }
        return desc.terms.map((term) => {
          if(term.base in base_optimization_functions) {
            return `${term.coeff}*${term.base}`
          } else {
            throw new Error(`Unknown optimization function: ${term.base}`)
          }
        }).join('+')
      }
    })

    const combination_fn_defs = Object.fromEntries(requested_optimization_function_descriptions.flatMap((desc, idx) => {
      const name = requested_optimization_function_names[idx]
      if(desc.type == 'single') {
        return []
      } else {
        return [[name, linearCombination(desc.terms)]]
      }
    }))

    let rosette_functions = {
      ...ROSETTE_FUNCTIONS,
      ...base_optimization_functions,
      ...combination_fn_defs
    }

    return {
      query: {
        "registry": rosetteRegistry, 
        "context_dependencies": rosetteContextNode,
        "options": {
          "check_acyclic": disallow_cycles,
          "minimization_criteria": requested_optimization_function_names
        },
        "functions": rosette_functions
      },
      prereleaseOrders: allPrereleaseOrders
    }
  }

  [_setConsistencyOnQuery] (query, requested_consistency) {
    query["functions"]["consistency"] = CONSISTENCY_RELATIONS[requested_consistency]
  }

  async [_buildReachableRegistry] (rootNode, has_min_cve) {
    this[_reachableNodesQueue] = [rootNode]
    this[_reachableRegistry] = {}
    this[_seenNodes] = {}
    this[_foundContextNode] = null

    let context = true

    let i = 0;

    while(this[_reachableNodesQueue].length > 0) {
      if(i % 100 == 0) {
        console.log(i + ' / ' + this[_reachableNodesQueue].length)
      }
      const node = this[_reachableNodesQueue].shift()
      this[_markNodeSeen](node)
      await this[_reachableNodesStep](context, node, has_min_cve)
      context = false
  ...
  [_markNodeSeen](node) {
    if(node.name in this[_seenNodes]) {
      if(node.version in this[_seenNodes][node.name]) {
        // console.log("Already seen: " + node.name + "@" + node.version)
        return false
      } else {
        this[_seenNodes][node.name][node.version] = true
      }
    } else {
      this[_seenNodes][node.name] = {}
      this[_seenNodes][node.name][node.version] = true
    }
    return true
  }
  
  async [_reachableNodesStep] (isContextNode, node, has_min_cve) {

    let typeFilter = null

    if(this[_rosetteOnlyExplore]) {
      // typeFilter = e => isContextNode ? true : e[1].type != 'dev'
      typeFilter = e => e[1].type == 'prod'
    ...
    const dependencies = Array.from(node.edgesOut)
      .filter(typeFilter)
      // .filter(e => isContextNode ? (e[1].type == 'prod' || e[1].type == 'dev') : (e[1].type == 'prod'))
      .flatMap(e => {
        try {
          const spec = npa.resolve(e[1].name, e[1].spec, e[1].from.path)
          return [{
            'packageToDependOn': e[1].name, 
            'constraintNpmSpec': spec, 
            'semverRange': (spec.type == 'range' || spec.type == 'version') ? new semver.Range(spec.fetchSpec, {loose: true}) : null}]
        } catch (error) {
          console.error(error)
          return []
        }
    ...
    if(isContextNode) {
      this[_foundContextNode] = dependencies
    } else {
      if(!(node.name in this[_reachableRegistry])) {
        this[_reachableRegistry][node.name] = {}
      }
      
      if(!(node.version in this[_reachableRegistry][node.name])) {
        this[_reachableRegistry][node.name][node.version] = {
          "dependencies": dependencies, 
          "node": node, 
          "semver": semver.parse(node.version),
          "cost_values": node.cost_values
        }
      }
    }
    ...
    await Promise.all(dependencies.map(dep => {
      return this[_fetchPackuments](dep.constraintNpmSpec).then(async pack => {
        ...
        let version_list = Object.keys(pack.versions)
        version_list.sort((v1, v2) => semver.compare(v2, v1, 'loose'))
        let version_oldnesses = Object.fromEntries(version_list.map((v_str, idx) => {
          if(version_list.length <= 1) {
            // If there is only 1 version, then we should have 0 cost for choosing it
            return [v_str, 0]
          } else {
            // Otherwise we proportionally receive cost in [0, 1]
            return [v_str, idx / (version_list.length - 1)]
          }
        }))

        
        let version_badnesses = null;
        if(has_min_cve) {
	  const url = `https://advisory-federico-codes.apps.in.ripley.cloud/api/cached/npm/${pack.name}`
          //const url = `https://advisory.federico.codes/api/cached/npm/${pack.name}`
          const vulns = await fetch(url, {retry: {retries: 10, randomize: true}}).then(res => {
          ...
          version_badnesses = Object.fromEntries(version_list.map((v_str) => {
            let badness = 0;
            for (const vuln of vulns) {
              const validRange = vuln.range.replace(', ', ' ')
              if (semver.satisfies(v_str, validRange, { loose: true, includePrerelease: true })) {
                badness += vuln.badness
              }
            }
            return [v_str, badness]
          }))
        }

        const versionPrereleases = Object.keys(pack.versions).flatMap(version => {
          const semverObj = semver.parse(version)
          return semverObj ? [semverObj.prerelease] : []
        })
        ...
        // prereleaseOrder : Map Prerelease Int
        const tmpPrereleaseOrder = buildPrereleaseOrder(versionPrereleases.concat(depPrereleases))

        const constraint = rosetteConstraintFromRange(dep.semverRange, tmpPrereleaseOrder)
        ...
        Object.entries(pack.versions).map(vers_pkg => {
          const jsonRosetteVersion = parseVersionString(vers_pkg[0], tmpPrereleaseOrder)
          if(!jsonRosetteVersion) {
            return
          }

          const tmpRosetteVersion = dsl.evalFunction(CONSTRAINT_PRIMITIVES, ROSETTE_FUNCTIONS, 'versionDeserialize', [jsonRosetteVersion])
          const pkg = vers_pkg[1]
          let newNode = this[_nodeFromPkg](pkg.name, pkg, null, legacyPeerDeps)
          let oldness = version_oldnesses[vers_pkg[0]]
          if(oldness === undefined) {
            throw new Error(`Oldness not found for ${vers_pkg[0]}`)          
          } 
          
          if(has_min_cve) {
            let badness = version_badnesses[vers_pkg[0]]
            if (badness === undefined) {
              throw new Error(`Badness not found for ${vers_pkg[0]}`)
            }
            newNode.cost_values = {'version_oldness': oldness, 'version_cve_badness': badness}
          } else {
            newNode.cost_values = {'version_oldness': oldness}
          }
          ...
          if(constraintFn(tmpRosetteVersion)) {
            if(this[_markNodeSeen](newNode)) {
              this[_reachableNodesQueue].push(newNode)
            }
          }
        })
      }, error => {
      ...
      // If there is a fetch error, we don't add the nodes to the rechable registry.
      // If it turns out that this failed fetch is necessary for the solve, then the solve will fail.
    }))
  ...
  async [_performRosetteQuery] (query) {
    ...
    console.log("Sending query to rosette: " + queryPath)
    if(this[_rosetteOnlyExplore]) {
      process.exit(0)
    } else {
      await execFile(this[_rosetteSolverPath], [queryPath, answerPath])

      const answer = JSON.parse(await fs.readFile(answerPath))
      console.log('Got: ' + answerPath)
  ...
  [_decodeRosetteAnswer] (answer, prereleaseOrders) {
    console.log("Decoding Rosette answer: " + answer)
    // return new Node({ name, pkg, parent, legacyPeerDeps })

    if(!answer.success) {
      throw new Error(answer.error)
    }

    const g = answer.graph
    const context_vertex = g.context_vertex
    const out_edges = g.out_edge_array

    const nodes = g.vertices.map((v, idx) => {
      if(idx == context_vertex) {
        return this.idealTree.target
      } else {
        const name = v.package // 'immutable'
        const version = v.version
        const realPrerelease = prereleaseOrders[name].orderToPrerelease(version.prerelease)
        const sem = literalSemver(version.major, version.minor, version.bug, realPrerelease)
        const versionString = sem.format()
        // const versionString = version.major + "." + version.minor + "." + version.bug
        return this[_reachableRegistry][name][versionString].node
      }
    })

    const visited = nodes.map((n) => false);

    // We do a BFS through the resolution graph starting at the context_vertex
    // in order to build a spanning tree for the graph. The tree edges
    // become the Arborist tree edges, and non-traversed edges become Arborist Links.
    let vertex_queue = [context_vertex];
    visited[context_vertex] = true;
    while (vertex_queue.length > 0) {
      const src = vertex_queue.shift();
      const srcNode = nodes[src];
      const edges = out_edges[src];
      for (let i = 0; i < edges.length; i++) {
        const dest = edges[i];
        const destNode = nodes[dest];
        if (visited[dest]) {
          new Link({ parent: srcNode, target: destNode })
        } else {
          destNode.parent = srcNode;
          visited[dest] = true;
          vertex_queue.push(dest);
        }
      }
    }
  }
  ...
  [_parseSettings] (options) {
    ...
    this[_useRosetteSolver] = !!options.rosette
    if(this[_useRosetteSolver]) {
      this[_rosetteSolverPath] = options.rosetteSolverPath
      this[_rosetteConsistencyArg] = options.rosetteConsistencyArg
      this[_rosetteMinimizeArg] = options.rosetteMinimizeArg
      this[_rosetteDisallowCyclesArg] = options.rosetteDisallowCyclesArg
      
      this[_rosetteOnlyExplore] = !!options.rosetteOnlyExplore
    }
...
`)
  ...
  async [_fetchPackuments] (spec) {
    const options = {
      ...this.options,
      avoid: this[_avoidRange](spec.name),
    }

    if (this[_packuments].has(spec.raw))
      return this[_packuments].get(spec.raw)
    else {
      this.log.silly('fetch packument', spec.raw)
      const p = pacote.packument(spec, options)
        .then(pack => {
          this[_packuments].set(spec.raw, pack)
          return pack
        })
      this[_packuments].set(spec.raw, p)
      return p
    }
  }

  [_nodeFromPkg] (name, pkg, parent, legacyPeerDeps) {
    return new Node({ name, pkg, parent, legacyPeerDeps })
  }
  ...
  [_nodeFromSpec] (name, spec, parent, edge) {
    ...
    return spec.type === 'directory'
      ...
      : this[_fetchManifest](spec)
        .then(pkg => this[_nodeFromPkg](name, pkg, parent, legacyPeerDeps), error => {
